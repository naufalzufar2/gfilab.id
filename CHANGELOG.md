...
Every time you ship a change, copy this block to the top of the file (right below `## [Unreleased]`, or turn `[Unreleased]` into a dated version if it's a real release):
...
**Versioning guide** (keep it simple — you don't need strict semver discipline for a single-product project like this):
- **MAJOR** (`2.0.0`) — big redesign, new core feature, breaking change to how results/data are stored
- **MINOR** (`1.1.0`) — new feature or page, meaningful visible change
- **PATCH** (`1.0.1`) — bug fix, copy change, small tweak

Delete any section (`Added`/`Changed`/`Fixed`/`Removed`) that's empty for that entry — don't leave blank headers in the published version.
